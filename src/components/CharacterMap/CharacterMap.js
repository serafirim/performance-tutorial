...
function itemize(text, transformer){
    const letters = text.split('')
      ...
      .reduce((collection, item) => {
        const letter = transformer ? transformer(item) : item // Replaced: "const letter = item.toLowerCase()"
        return {
          ...collection,
          [letter]: (collection[letter] || 0) + 1
        }
      }, {})
    return Object.entries(letters) // Replaced: "return letters"
     .sort((a, b) => b[1] - a[1])
  }
...
function CharacterMap({ showExplanation, text, transformer }) {
  ...
  return(
    <div>
        {showExplanation && 
          <p>
            This displays a list of most common characters.
          </p>
        }
